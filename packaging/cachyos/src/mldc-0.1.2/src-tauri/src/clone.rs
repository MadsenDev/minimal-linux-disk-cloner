...
fn run_clone_process(app: &AppHandle, config: &CloneConfig) -> CloneResult {
    ...
    loop {
        let line = match read_progress_record(&mut reader) {
            Ok(Some(line)) => line,
            Ok(None) => break,
            Err(err) => {
                progress
...
fn read_progress_record<R: BufRead>(reader: &mut R) -> Result<Option<String>, std::io::Error> {
    ...
    loop {
        ...
        if let Some(idx) = available.iter().position(|byte| *byte == b'\r' || *byte == b'\n') {
            buffer.extend_from_slice(&available[..idx]);

            let delimiter = available[idx];
            let mut consume_len = idx + 1;
            if delimiter == b'\r' && available.get(idx + 1) == Some(&b'\n') {
                consume_len += 1;
            }
...
mod tests {
    ...
    #[test]
    fn reads_carriage_return_delimited_progress_records() {
        let mut reader = Cursor::new(
            b"1048576 bytes (1.0 MB, 1.0 MiB) copied, 1.0 s, 1.0 MB/s\r2097152 bytes (2.1 MB, 2.0 MiB) copied, 2.0 s, 1.0 MB/s\n"
                .to_vec(),
        );

        let first = super::read_progress_record(&mut reader)
            .unwrap()
            .expect("expected first record");
        let second = super::read_progress_record(&mut reader)
            .unwrap()
            .expect("expected second record");
        let end = super::read_progress_record(&mut reader).unwrap();

        assert!(first.starts_with("1048576 bytes"));
        assert!(second.starts_with("2097152 bytes"));
        assert!(end.is_none());
    }
