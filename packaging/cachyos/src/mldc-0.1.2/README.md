...
The packaged `mldc` launcher now tries to request elevated privileges through `pkexec` on startup. If the desktop auth prompt is cancelled or `pkexec` is unavailable, the app still opens in normal user mode for device discovery and UI checks, but starting a real clone will fail.
...
For actual real-device testing from the repo, build the debug app and run the binary with a desktop auth helper such as `pkexec`:
...
pkexec env \
  ...
  ./src-tauri/target/debug/mldc-tauri
```
...
Native CachyOS / Arch package from the current working tree:

```bash
./packaging/cachyos/build-package.sh
```

This writes a real pacman package to `packaging/cachyos/` as
`mldc-0.1.2-1-x86_64.pkg.tar.zst`.

Install it with:

```bash
sudo pacman -U packaging/cachyos/mldc-0.1.2-1-x86_64.pkg.tar.zst
```

On KDE Plasma Wayland, the packaged launcher automatically sets
`WEBKIT_DISABLE_DMABUF_RENDERER=1` to avoid the common WebKitGTK/Tauri
`Error 71 (Protocol error) dispatching to Wayland display` crash on startup.
It also attempts a `pkexec` relaunch while preserving the active desktop
session variables so the packaged app can open with a Polkit password prompt
instead of requiring `sudo mldc`.
...
- `packaging/cachyos/` — native pacman packaging for CachyOS / Arch
