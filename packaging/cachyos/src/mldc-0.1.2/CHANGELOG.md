...
- Made the current project version explicit in the README while keeping the app manifests aligned at `0.1.1`.
...
- Generated the full Tauri icon set from the existing `assets/mldc.svg` logo so the desktop app now uses the real project icon instead of the placeholder.
- Built the optimized Tauri desktop binary against the updated icon assets for Linux use.
- Added a native CachyOS/Arch `PKGBUILD` and local build helper that package the app as a real `.pkg.tar.zst` pacman package.
- Added CachyOS/Arch package build and install instructions to the README.
- Added a packaged launcher wrapper that sets `WEBKIT_DISABLE_DMABUF_RENDERER=1` on Wayland sessions to avoid the common WebKitGTK/Tauri startup crash on KDE Plasma.
- Updated the packaged Linux launcher to attempt a `pkexec` relaunch with the active KDE/Wayland session variables preserved, so starting `mldc` can raise a Polkit auth prompt instead of requiring `sudo mldc`.
- Fixed the CachyOS package helper so the generated source tarball skips `packaging/cachyos/src`, `packaging/cachyos/pkg`, and prior package archives instead of trying to repackage deleted build-output trees.
- Tightened disk warning detection so only actually mounted partitions count as mounted, and the smaller-target warning now reports more precise sizes when rounded GiB labels would otherwise look identical.
- Fixed clone progress streaming so `dd status=progress` updates terminated with carriage returns are emitted live instead of waiting for a newline or process exit.
- Bumped the project version metadata, package recipe, frontend fallback, and README references from `0.1.0` to `0.1.1`.
- Reworked Settings into a tabbed preferences modal backed by a versioned settings object, added a default-on safety guard that blocks starts on `Danger` warnings, and introduced power-user options for exact byte display, warning diagnostics, and default-expanded run detail panels.
- Pinned the Tauri dev server host and `devUrl` to `127.0.0.1` so `tauri dev` does not white-screen on systems where `localhost` resolves to IPv6 loopback first.
- Added a moving sheen and subtle pulse to the progress bar so clone and verification runs feel active even between streamed progress updates, while still respecting reduced-motion and background-motion-disabled modes.
- Applied the same `WEBKIT_DISABLE_DMABUF_RENDERER=1` workaround used by the packaged launcher to the `tauri:dev` scripts so KDE Plasma development runs avoid the WebKitGTK white-window crash path too.
- Reduced packaged UI sluggishness by moving progress/result rendering into focused component boundaries, deferring live log updates, and toning down the heaviest animated background and progress-bar filter effects.
- Replaced the native Linux titlebar with a custom MLDC titlebar integrated into the existing header, using an undecorated Tauri window plus custom drag, double-click maximize/restore, minimize, maximize, and close controls.
- Reworked the custom titlebar layout so it now spans the full window width, moves Settings and Refresh to the left edge, centers the MLDC branding in the draggable region, and keeps the window controls isolated on the right.
- Bumped the project version metadata, package recipe, frontend fallback, and README package references from `0.1.1` to `0.1.2`.
